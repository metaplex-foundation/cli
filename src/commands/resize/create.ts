...
export default class ResizeCreate extends TransactionCommand<typeof ResizeCreate> {
...

{
  ...
  static override examples = [
    '$ mplx resize create --config ./distribution-config.json',
    '$ mplx resize create --name "Community Airdrop" --mint "TokenMint123..." --totalClaimants 1000 --startTime "2024-01-01T00:00:00Z" --endTime "2024-12-31T23:59:59Z" --merkleRoot "base58EncodedRoot"',
    '$ mplx resize create --name "Reward Distribution" --mint "TokenMint123..." --totalClaimants 500 --startTime "2024-06-01T12:00:00Z" --endTime "2024-06-30T12:00:00Z" --merkleRoot "base58EncodedRoot" --distributionType legacy-nft',
  ]
  ...
  public async run(): Promise<void> {
    const {flags} = await this.parse(ResizeCreate)
    ...
    try {
      ...
      const distributionType =
        config.distributionType === 'legacy-nft' ? DistributionType.LegacyNft : DistributionType.Wallet

      // Parse allowed distributor
      const allowedDistributor =
        config.allowedDistributor === 'recipient' ? AllowedDistributor.Recipient : AllowedDistributor.Permissionless
      ...
      const result = await umiSendAndConfirmTransaction(
        this.context.umi,
        createDistribution(this.context.umi, {
          allowedDistributor,
      ...
      // Display results
      this.log('')
      this.logSuccess(`Distribution created: ${distributionPda[0]}`)
