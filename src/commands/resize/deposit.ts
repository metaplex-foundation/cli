...
export default class ResizeDeposit extends TransactionCommand<typeof ResizeDeposit> {
...
This command deposits tokens from your wallet into a distribution created by mplx resize create.
The distribution must be active and you must have the tokens in your wallet.`

  static override examples = [
    '$ mplx resize deposit DistroAddress123... --amount 1.0',
    '$ mplx resize deposit DistroAddress123... --basisAmount 1000000',
  ]
  ...
  static override usage = 'resize deposit [DISTRIBUTION] [FLAGS]'

  public async run(): Promise<void> {
    const { args, flags } = await this.parse(ResizeDeposit)
    ...
    try {
      ...
      const result = await umiSendAndConfirmTransaction(this.context.umi, depositIx)
