...
echo "$UNLOCK_OUT" | grep -q "Claim Count: 0" && pass "Claim count is zero" || fail "Claim count is zero"
echo "$UNLOCK_OUT" | grep -q "Amount Claimed: 0" && pass "Amount claimed is zero" || fail "Amount claimed is zero"
